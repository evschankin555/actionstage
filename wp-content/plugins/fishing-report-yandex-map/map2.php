...
$posts = $wpdb->get_results("SELECT id  FROM $wpdb->posts WHERE post_status = 'publish'
   AND post_type='post' ORDER BY comment_count DESC LIMIT 0,2");
echo '<pre>';
//_sfm_area=Богородский%20городской%20округ&_sfm_season=Декабрь
/*
_sfm_country
_sfm_province_1
_sfm_province_2
...
echo $sql_query = "SELECT $wpdb->posts.id, $wpdb->posts.post_content, $wpdb->posts.post_title 
...
function get_all(){
    global $wpdb;
    $posts = $wpdb->get_results("SELECT id, post_content, post_title  FROM $wpdb->posts WHERE post_status = 'publish'
   AND post_type='post' ORDER BY comment_count DESC LIMIT 0,10000");
    return $posts;
}
function get_by_tag($tag = 'moskovskaya-oblast'){
    global $wpdb;
    $id_tag = get_terms([
        'taxonomy' => 'post_tag',
        'slug' => $tag
    ...
    $sql_query = "SELECT * from wp_search_filter_term_results where field_value={$id_tag[0]->term_taxonomy_id} limit 10 ";
    $result =  $wpdb->get_results($sql_query,ARRAY_A);
    $ids = explode(',', $result[0]['result_ids']);

    $posts = $wpdb->get_results("SELECT id, post_content, post_title  FROM $wpdb->posts WHERE post_status = 'publish'
   AND post_type='post' AND id in ({$result[0]['result_ids']}) ORDER BY post_date DESC   ");
    return $posts;
}

$sql_query = "SELECT * from $wpdb->postmeta WHERE  post_id in({$posts[0]->id})";
$result =  $wpdb->get_results($sql_query,ARRAY_A);
print_r($result);
*/
